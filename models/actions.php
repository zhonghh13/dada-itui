...
class actions_class extends AWS_MODEL
{
	public function home_activity($uid, $limit = 10)
	{
		...
		if ($user_focus_topics_ids = $this->model('topic')->get_focus_topic_ids_by_uid($uid))
		{
			...
			{
				if ($user_focus_article_topics_info = $this->model('article')->get_topic_info_by_article_ids($user_focus_topics_article_ids))
				{
					foreach ($user_focus_article_topics_info AS $key => $user_focus_topics_info_by_article)
					{
						foreach ($user_focus_topics_info_by_article AS $_key => $_val)
						{
							if (!in_array($_val['topic_id'], $user_focus_topics_ids))
							{
								unset($user_focus_article_topics_info[$key][$_key]);
							}
						}
					}
				}
		...
		foreach ($action_list as $key => $val)
		{
			...
			{
				if ($user_focus_article_topics_info[$val['associate_id']] AND !in_array($action_list[$key]['uid'], $user_follow_uids) AND $action_list[$key]['uid'] != $uid)
				{
					$topic_info = end($user_focus_article_topics_info[$val['associate_id']]);				
				}
				else
				{	
					unset($topic_info);

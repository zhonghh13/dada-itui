		...
		foreach ($data AS $key => $val)
		{
			if ($val['extend_count'] > 1)
			{
				switch ($val['model_type'])
				{
					...
					case self::CATEGORY_ARTICLE:
						$data[$key]['message'] = $val['extend_count'] . ' ' . AWS_APP::lang()->_t('项关于笔记') . ' <a href="' . $val['key_url'] . '">' . $val['title'] . '</a>';
				...
				foreach($val['extend_details'] AS $action_type => $extend)
				{
					...
					switch ($action_type)
					{
						...
						case self::TYPE_ARTICLE_COMMENT_AT_ME:
							$data[$key]['extend_message'][] = AWS_APP::lang()->_t('他们在笔记中的评论提到了你') . ': ' . $users_list;

							break;

						case self::TYPE_ARTICLE_NEW_COMMENT:
							$data[$key]['extend_message'][] = AWS_APP::lang()->_t('%s 个新回复, 评论来自', $extend['count']) . ': ' . $users_list;

							break;

						case self::TYPE_NEW_ANSWER:
							$data[$key]['extend_message'][] = AWS_APP::lang()->_t('%s 个新回复, 回复来自', $extend['count']) . ': ' . $users_list;

							break;

						case self::TYPE_ANSWER_COMMENT:
							$data[$key]['extend_message'][] = AWS_APP::lang()->_t('%s 个新评论, 评论来自', $extend['count']) . ': ' . $users_list;

							break;

						case self::TYPE_ANSWER_AGREE:
							$data[$key]['extend_message'][] = AWS_APP::lang()->_t('%s 个新赞同, 赞同来自', $extend['count']) . ': ' . $users_list;

							break;

						case self::TYPE_ANSWER_THANK:
							$data[$key]['extend_message'][] = AWS_APP::lang()->_t('%s 个新感谢, 感谢来自', $extend['count']) . ': ' . $users_list;

							break;

						case self::TYPE_MOD_QUESTION:
							$data[$key]['extend_message'][] = AWS_APP::lang()->_t('%s 次编辑问题, 编辑者为', $extend['count']) . ': ' . $users_list;
			...
			else
			{
				switch ($val['action_type'])
				{
					...
					case self::TYPE_ARTICLE_NEW_COMMENT:
						$data[$key]['message'] = '<a href="' . $val['p_url'] . '">' . $val['p_user_name'] . '</a> ' . AWS_APP::lang()->_t('评论了笔记') . ' <a href="' . $val['key_url'] . '">' . $val['title'] . '</a>';
					...
					case self::TYPE_ARTICLE_COMMENT_AT_ME:
						$data[$key]['message'] = '<a href="' . $val['p_url'] . '">' . $val['p_user_name'] . '</a> ' . AWS_APP::lang()->_t('在笔记') . ' <a href="' . $val['key_url'] . '">' . $val['title'] . '</a> ' . AWS_APP::lang()->_t('评论中回复了你');
					...
					case self::TYPE_ARTICLE_APPROVED:
						$data[$key]['message'] = AWS_APP::lang()->_t('你发起的笔记 %s 审核通过', '<a href="' . $val['key_url'] . '">' . $val['title'] . '</a>');

						break;

					case self::TYPE_ARTICLE_REFUSED:
						$data[$key]['message'] = AWS_APP::lang()->_t('你发起的笔记 %s 审核未通过', $val['title']);

...
class article_class extends AWS_MODEL
{
	...
	public function get_topic_info_by_article_ids($article_ids)
	{
		...
		if ($topic_relation = $this->fetch_all('topic_relation', 'item_id IN(' . implode(',', $article_ids) . ") AND `type` = 'article'"))
		{
			...
			foreach ($topic_relation AS $key => $val)
			{
				$topics_by_articles_ids[$val['item_id']][] = array(

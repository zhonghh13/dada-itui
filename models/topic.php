...
class topic_class extends AWS_MODEL
{
	...
	//每个topic推荐三个问题
	public function get_questions_by_topic_ids($topic_ids, $type)
	{
		if (!is_array($topic_ids) OR sizeof($topic_ids) == 0)
		{
			return false;
		}
		...
		if (!$item_questions = $this->fetch_all('topic_relation', "topic_id IN(" . implode(',', $topic_ids) . ") AND `type` = '" . $this->quote($type) . "'", "add_time DESC"))
		{
			foreach ($topic_ids AS $topic_id)
			{
				if (!$result[$topic_id])
		...
		foreach ($item_questions AS $key => $val)
		{
			$item_ids[] = $val['item_id'];
		}

		$question_infos = $this->model('question')->get_question_info_by_ids($item_ids);
		...
		foreach ($item_questions AS $key => $val)
		{
			$result[$val['topic_id']][] = $question_infos[$val['item_id']];
		}
		...
		foreach ($result as $key => $value) {
			$result[$key] = array_slice($result[$key], 0, 3);
		}
	...
	public function unset_recommend($topic_id)
	{
		$this->update('topic', array(
			'is_recommend' => 0
		), 'topic_id = ' . intval($topic_id));

...
class posts_class extends AWS_MODEL
{
		...
		else
		{
			switch ($post_type)
			{
				...
				case 'project':
					...
					break;
				case 'topic':
					$result =  $this->fetch_row('topic', 'topic_id = '. intval($post_id));
					break;
			}
		...
		switch ($post_type)
		{
			...
			case 'topic':
				$data = array(
					...
					'category_id' => 0,
					'is_recommend' => $result['is_recommend'],
					'view_count' => $result['discuss_count'],
					'anonymous' => 0,
					'popular_value' => 0,
					'uid' => 0,
	...
	public function process_explore_list_data($posts_index)
	{
		...
		{
			// echo "&&&&&&&&&&&&";
			$questions_info = $this->model('topic')->get_questions_by_topic_ids($topics_id, 'question');

			$topics_info = $this->model('topic')->get_topics_by_ids($topics_id);
		}
		...
		foreach ($posts_index as $key => $data)
		{
			...
			switch ($data['post_type'])
			{
				case 'question':
					...
					{
						// echo "string".$question_infos[$data['post_id']]['published_uid'];
						$explore_list_data[$key]['answer_info']['user_info'] = $users_info[$question_infos[$data['post_id']]['published_uid']];
					}
				...
				case 'article':
					...
					$explore_list_data[$key]['answer_info']['user_info'] = $users_info[$data['uid']];
					break;
				...
				//干什么用的？？？？？？？？？？？&&&&
				case 'topic':
					$explore_list_data[$key] = $topics_info[$data['post_id']];
					break;
			}
			...
			if ($data['post_type'] == 'topic'){
				// echo "%%%%%%%%%%%";
				$explore_list_data[$key]['topics'] = $topics_info[$data['post_id']];
				$explore_list_data[$key]['questions'] = $questions_info[$data['post_id']];
			}else
			{
				$explore_list_data[$key]['topics'] = $topic_infos[$data['post_type']][$data['post_id']];
			}

			$explore_list_data[$key]['user_info'] = $users_info;
		// }
		}

...
class user extends AWS_ADMIN_CONTROLLER
{
    ...
    public function user_generate_action()
    {
        $this->crumb(AWS_APP::lang()->_t('生成邀请码'), "admin/user/invite_code/user_generate/");

        TPL::assign('menu_list', $this->model('admin')->fetch_menu_list(408));

        TPL::output('admin/user/generate');
    }

    public function invite_code_action()
    {
        // TPL::assign('job_list', $this->model('work')->get_jobs_list());
        TPL::assign('list', $this->model('admininvitation')->fetch_all('admininvitation', null, 'add_time DESC'));

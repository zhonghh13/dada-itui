...
class ajax extends AWS_ADMIN_CONTROLLER
{
    ...
    public function article_manage_action()
    {
        ...
        switch ($_POST['action'])
        {
            ...
            case 'recommend':
                foreach ($_POST['article_ids'] as $article_id) {
                    // echo $question_id;
                    $this->model('article')->set_admin_recommend($article_id);    
                }
                H::ajax_json_output(AWS_APP::RSM(null, 1, null));
            break;
            ...
            case 'cancel_recommend':
                foreach ($_POST['article_ids'] as $article_id) {
                    // echo $question_id;
                    $this->model('article')->unset_recommend($article_id);    
                }
                H::ajax_json_output(AWS_APP::RSM(null, 1, null));
            break;
            case 'headline':
                foreach ($_POST['article_ids'] as $article_id) {
                    // echo $question_id;
                    $this->model('article')->set_headline($article_id);    
                }
                H::ajax_json_output(AWS_APP::RSM(null, 1, null));
            break;
        }
    ...
    public function question_manage_action()
    {
        ...
        switch ($_POST['action'])
        {
            ...
            case 'recommend':
                foreach ($_POST['question_ids'] as $question_id) {
                    // echo $question_id;
                    $this->model('question')->set_admin_recommend($question_id);    
                }
                H::ajax_json_output(AWS_APP::RSM(null, 1, null));
            break;
            ...
            case 'cancel_recommend':
                foreach ($_POST['question_ids'] as $question_id) {
                    // echo $question_id;
                    $this->model('question')->unset_recommend($question_id);    
                }
                H::ajax_json_output(AWS_APP::RSM(null, 1, null));
            break;
        }
        ...
        switch($_POST['action'])
        {
            ...
            case 'recommend':
                foreach ($_POST['topic_ids'] as $topic_id) {
                    // echo $question_id;
                    $this->model('topic')->set_admin_recommend($topic_id);    
                }
                H::ajax_json_output(AWS_APP::RSM(null, 1, null));
            break;
            ...
            case 'cancel_recommend':
                foreach ($_POST['topic_ids'] as $topic_id) {
                    // echo $question_id;
                    $this->model('topic')->unset_recommend($topic_id);    
                }
                H::ajax_json_output(AWS_APP::RSM(null, 1, null));
            break;
        }
    ...
    public function save_admininvitation_action()
    {
        if ($_POST['string_input'] AND $_POST['diy_count'])
        {
            $this->model('admininvitation')->add_invitation($_POST['string_input'],time(),$_POST['diy_count'],$_POST['diy_count']);
            H::ajax_json_output(AWS_APP::RSM(array(
                    'url' => get_js_url('/admin/user/invite_code/')
                ), 1, null));
        }
        else
        {
            H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('输入错误')));
        }
    }
    public function save_admininvitation_rand_action()
    {
        if ($_POST['random_count'])
        {
             // echo intval($_POST['random_count']);
            for ($i = 0; $i < intval($_POST['random_count']); $i++)
            {
                $invitation_code = $this->model('admininvitation')->get_unique_invitation_code();
                $this->model('admininvitation')->add_invitation($invitation_code,time());
            }

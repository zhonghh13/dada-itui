...
class main extends AWS_CONTROLLER
{
	...
	public function index_action()
	{
			...
			default:
				...
				if ($posts_list = $this->model('posts')->get_posts_list(null, 1, get_setting('contents_per_page'), 'new', $contents_related_topic_ids))
				{
					foreach ($posts_list AS $key => $val)
					{
						...
						// if (!$val['question_id'] AND !$val['answer_count'])
						// {
						// 	$posts_list[$key]['message'] = FORMAT::parse_bbcode($val['message']);
						// }
					}
				}
			// 	echo "8888";
		...
		// }
				...
				if ($posts_list = $this->model('posts')->get_posts_list(null, 1, get_setting('contents_per_page'), null, $contents_related_topic_ids, null, null, 30, 1))
				{
					foreach ($posts_list AS $key => $val)
					{
						if ($val['answer_count'])
	...
	public function index_square_action()
	{
		...
		if (is_mobile())
		{
			HTTP::redirect('/m/topic/');
				...
				// 边栏热门用户
		if (TPL::is_output('block/sidebar_hot_users.tpl.htm', 'topic/square'))
		{
		...
		if ($today_topics = rtrim(get_setting('today_topics'), ','))
		{
			...
			// foreach ($today_topic as $key => $value) {
			// 	echo $key.":".$value."\n";
			// }
	...
	public function home_action()
	{
		...
		//热门用户
		if (TPL::is_output('block/sidebar_hot_users.tpl.htm', 'topic/home'))
		{
			TPL::assign('sidebar_hot_users', $this->model('module')->sidebar_hot_users($this->user_id, 5));
		}

		// if ($posts_list = $this->model('posts')->get_posts_list(null, $_GET['page'], get_setting('contents_per_page'), $_GET['sort_type'], null, $category_info['id'], $_GET['answer_count'], $_GET['day'], 2))
		// {
		...
		$this->crumb(AWS_APP::lang()->_t('话题'), '/topic/home/');
		TPL::import_js('js/app/topic_home.js');
		TPL::output('topic/home');
	}
}
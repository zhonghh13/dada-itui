...
class ajax extends AWS_CONTROLLER
{
	...
	public function get_topics_list_action()
	{
		if ($posts_list = $this->model('posts')->get_posts_list('topic', $_GET['page']+1, get_setting('contents_per_page'), $_GET['sort_type'], null, $category_info['id'], $_GET['answer_count'], $_GET['day'], 1))
		{
			
		}

		// foreach ($posts_list as $key => $value) {
		// 	foreach ($value['topics'] as $e_key => $e_value) {
		// 		echo $e_key.":".$e_value."^^^";
		// 	}
		// }
		...
		if (is_mobile())
		{
			TPL::output('m/ajax/recommend_topics_list');
		}
		else
		{
			TPL::output('topic/ajax/recommend_topics_list');
		}
	}
}